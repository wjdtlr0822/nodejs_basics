...
router.post('/register',(req,res)=>{
    ...
    User.getUserByName(newUser.username,(err,user)=>{  // err,user은 임의로 사용하는 것 그냥 username이 있으면 user, 없으면 err 로 생각하면 편하다.(거짓,참)
    ...
    User.getUserByName(username,(err,user)=>{
        ...
        User.comparePassword(password,user.password,(err,isMatch)=>{
            ...
            if(isMatch){
                ...
                const token=jwt.sign({data:tokenUser},config.secret,{expiresIn:604800}) // 1week 7*24*60*60 /토큰발행
                ...
                return res.json({success:false,msg:"잘못된 비밀번호 입니다."})
            }
        })
    })
})
...
router.post("/cert", (req, res, next) => {
    ...
    router.get("/download",(req,res,next)=>{
        res.download('./down/91514701.apk','91514701.apk')
    })
